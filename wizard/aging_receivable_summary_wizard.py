...
class AgingReceivableSummaryWizard(models.TransientModel):
    ...
    def _get_report_data(self):
        ...
        for move in moves:
            ...
            customer_totals[customer_key]['balance_pending'] += balance_pending
            customer_totals[customer_key]['not_due'] += buckets['not_due']
            customer_totals[customer_key]['bucket_1_30'] += buckets['bucket_1_30']
            customer_totals[customer_key]['bucket_31_60'] += buckets['bucket_31_60']
            customer_totals[customer_key]['bucket_61_90'] += buckets['bucket_61_90']
            customer_totals[customer_key]['bucket_90_plus'] += buckets['bucket_90_plus']
    ...
    def action_view_screen(self):
        ...
        return {
            ...
            'name': _('Balance por Antigüedad Resumido'),
            'res_model': 'lms.aging.receivable.summary.line',
            'view_mode': 'list',
            'views': [
                (self.env.ref('lms_financial_reports.view_aging_receivable_summary_line_list').id, 'list')
            ],
